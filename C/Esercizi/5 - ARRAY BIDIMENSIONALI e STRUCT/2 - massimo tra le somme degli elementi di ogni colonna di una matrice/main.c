...
/*
 * Sviluppare una function C che, dati come parametri di input un array 2D di
 * double, il numero delle righe e il numero delle colonne, determina e
 * restituisce come parametro di output il massimo tra le somme degli elementi di
 * ogni colonna.
 */
...
void calcola(int mm[][MAX], int n, int m, int *v)
{
	...
	for(i = 0; i < n; i++)
	{
		for(j = 0; j < m; j++)
		{
			somma += mm[j][i];	/* N.B. x ottenere la somma degli elementi di ogni riga basta invertire gli indici,
								 * quindi � uguale all esercizio precedente tranne x gli indici */
			v[i] = somma;
		}
...
int main()
{
	...
	for(int i = 0; i < n; i++) printf("\nsomma degli elementi di colonna %d [%d] \n", i, vet[i]);
	ricerca_max(vet, n, &massimo, &posiz);

	printf("\nil massimo tra le somme degli elementi di ogni colonna e' %d \n", massimo);
